...
<?php
		$id_reponse =$donnees2['id_reponse'];
		$newlabel_reponse = $_POST["$id_reponse"];
		$newvalidite= $_POST['bonne_reponse'.$donnees2['id_reponse']];
		
		//$req.=$id_reponse;
		$req = $linkpdo->prepare('UPDATE reponse SET label_reponse, validite VALUES :newlabel_reponse, :newvalidite where id_reponse = $id_reponse');
			///Exécution de la requête
		$req->execute(array('newlabel_reponse' => $newlabel_reponse,'newvalidite' => $newvalidite));
