...
<body>
	<div class="row">
		...
		<div class="col-sm-6">
			...
			<form class=" form" action="qcm_repondre_qcm.php" method="POST">
...
$reponse = $linkpdo->query("SELECT label_qcm, id_qcm_fk, id_user_fk FROM qcm");?>
				<select class="form-control" title="choisissez dans la liste" name="liste_de_qcm">
					<?php
foreach ($reponse as $data)
{echo '<option value="' . $data['id_qcm_fk'] . '">' . $data['label_qcm'] . '</option>';}
?></select>
				<input class="form-control" type="submit" value="repondre au qcm?" name="repondre_qcm">
			</form>
			<br>
	...
	while ($donnees = $req_afficher_qcm->fetch()) {?>
			<br>
			<form class="form" action="traitement_valider_reponse.php" method="POST">
